...
- `averagingLayer:Bool=false`: sets whether or not to only apply just the
                               averaging filter.
"""
function shearingLayer(inputSize::Union{Int,NTuple}; scale = -1, shearLevel = scale, bias = nothing, dType = Float32, σ = abs, trainable = false, plan = true, boundary = Pad(-1, -1), averagingLayer = false)
